...
The fiscal year starts in October, so FY26 Q1 = Oct–Dec 2025 and FY26 Q3 =
Apr–Jun 2026.

Periods can be given as a fiscal quarter (`FY26Q3`), a calendar quarter
(`2026Q2`), or explicit `--from`/`--to` dates — a date range that happens to be a
whole fiscal quarter is still named `FY26Q3` in the output files.
...
- `src/period.ts` — fiscal and calendar quarter parsing, month enumeration.
...
A Play Console admin grants access in **Users and permissions → Invite new user**,
pasting the service account's own email address. Two things that are easy to get
wrong:

- **"View app information" must be set to Global**, i.e. account-level. An
  app-scoped grant — even one restricted to Bloom Reader specifically — does *not*
  give bulk-report access, and produces a bare `storage.objects.list` /
  `storage.buckets.get` denial that looks like no grant at all.
- **There is nothing for the service account to accept.** It has no inbox; the
  admin's grant takes effect immediately. A grant that appears not to work is
  misconfigured, not pending.
...
- **Bloom Reader Installs is `Daily Device Installs` from the bulk reports**, and
  ...
  **This is the one column that cannot be collected until the bulk-reports
  permission is granted.** Reading those CSVs from a local folder was a stopgap
  while the metric was being pinned down and has been removed; the bucket is the
  only route now. A run before the grant lands leaves Installs blank and says why.
