...
/**
 * Builds a Period from a calendar quarter (`2025Q4` = Oct-Dec 2025) or a fiscal
 * quarter (`FY26Q1`). Throws on anything it does not recognise -- we would
 * rather fail than silently report the wrong three months.
 */
export const parseQuarter = (spec: string): Period => {
    ...
    if (calendar) {
        ...
        return quarterFromAbsoluteMonth(startMonthAbsolute, spec, spec);
    }

    throw new Error(
        `Unrecognised quarter "${spec}". Use a calendar quarter like 2025Q4, a fiscal quarter like FY26Q1, or pass --from and --to.`
    );
};
