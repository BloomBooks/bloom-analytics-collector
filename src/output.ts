...
import { METRIC_ORDER } from "./metrics.js";
...
/**
 * Reads the layout, refusing anything self-inconsistent. The `headers` list is
 * only there to catch drift: if it stops lining up with `columns`, every paste
 * would land one cell sideways, so a mismatch is an error rather than a warning.
 */
export const loadDashboardLayout = async (
    ...
    // Validate the column names. A misspelt metric matches nothing, so it would
    // be treated exactly like a `null` "never touch" column -- silently blanking
    // that column of the dashboard on the next paste.
    for (const [index, column] of (
        parsed.columns as (string | null)[]
    ).entries()) {
        if (column === null) continue;
        if (!METRIC_ORDER.includes(column as MetricId)) {
            throw new Error(
                `${path}: "columns[${index}]" is "${column}", which is not a known metric. Expected null or one of ${METRIC_ORDER.join(", ")}.`
            );
        }
    }
...
/**
 * Builds the single grid for a quarter, or null if the layout cannot support one
 * (unknown or non-consecutive product rows -- a rectangle can only land on
 * adjacent rows).
 */
