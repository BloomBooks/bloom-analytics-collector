...
const fetchReleaseTags = async (repo: string): Promise<DatedTag[]> => {
    ...
    const tags = (await response.json()) as TagRef[];
    ...
    // Tag names do not sort meaningfully (v3.4.68 postdates v3.4.8), so order by
    // date and let the date decide what "latest" means.
