...
Options
  --quarter <FY26Q3>         reporting period (fiscal; FY26 starts Oct 2025)
...
Filled ${merged.applied} cell(s) from ${manual.path}.`
        ...
        for (const clash of merged.superseded) {
            console.log(`  ignored, we collect this ourselves: ${clash}`);
        }
        for (const stray of merged.notApplicable) {
            console.log(
                `  ignored, this column does not apply to that product: ${stray}`
            );
        }
        // Deliberately a different heading, and a louder one. These are cells the
        // file could have filled but must not, because the source FAILED this run.
        // Printing them under "we collect this ourselves" told whoever is running a
        // quarter that a failure was a success, and left them to notice a blank cell
        // on their own.
        for (const blocked of merged.refused) {
            console.log(
                `  NOT filled -- the source failed and a typed-in number would hide that: ${blocked}`
            );
        }
    } else {
        console.log(
