...
/** Where a run's three files ended up, plus the cells still needing a person. */
...
/**
 * Writes a quarter's results to files rather than leaving them in the terminal: a
 * paste grid to open and copy wholesale, a report recording every value and where
 * it came from, and the raw run for replaying.
 *
 * Files are named for the quarter and kept, so the folder accumulates a record of
 * past quarters. Provenance is sanitised on the way out, because these files are
 * committed and this repo is public -- which also means a value that came from a
 * source no longer reachable cannot be regenerated, only preserved.
 */
